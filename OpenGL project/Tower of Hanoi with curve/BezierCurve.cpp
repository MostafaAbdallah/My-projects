...
BezierBernsteinCurve::BezierBernsteinCurve()
 : controlPoints() , curvePoints() , selectedControlPoints() , constructionPoints(){
...
glm::vec3& BezierBernsteinCurve::getControlPoint(const unsigned int& index){
...
void BezierBernsteinCurve::drawConstructionPolygon(const bool& isStippled , const float& lineWidth , glm::vec4 lineColor , const int& speed){

    float currentWidth,currentColor[4],currentSize;
    ...
    if ( controlPoints.size() > 2 ){
        constructionPoints.resize(controlPoints.size()-2);

        for ( auto i = 0u; i < constructionPoints.size(); ++i )
            constructionPoints[i].resize(controlPoints.size() - (i+1));
    
        /*std::cout << "CP= " << controlPoints.size() << std::endl;
        std::cout << "ConP= " << constructionPoints.size() << std::endl;

        for ( auto i = 0u; i < constructionPoints.size(); ++i )
            std::cout << "L" << i+1 << "= " << constructionPoints[i].size() << std::endl;*/

        for ( auto i = 1u; i < controlPoints.size(); ++i )
                constructionPoints[0][i-1] = lerp(controlPoints[i-1],controlPoints[i],t);

        for ( auto k = 1u; k < constructionPoints.size(); ++k )
            for ( auto i = 1u; i < constructionPoints[k-1].size(); ++i )
                constructionPoints[k][i-1] = lerp(constructionPoints[k-1][i-1],constructionPoints[k-1][i],t);


        glm::vec3 p = lerp(constructionPoints[constructionPoints.size()-1] [0],
                            constructionPoints[constructionPoints.size()-1] [1],t);
    ...
    for ( auto k = 0u; k < constructionPoints.size(); ++k ){

        for ( auto i = 1u; i < constructionPoints[k].size(); ++i ){

            glBegin(GL_LINE_STRIP);
                glVertex2f(constructionPoints[k][i-1].x,constructionPoints[k][i-1].y); 
                glVertex2f(constructionPoints[k][i].x,constructionPoints[k][i].y); 
            glEnd();
        }
    }
...
/*void BezierBernsteinCurve::drawVectorAddition(float t){
...
void BezierBernsteinCurve::resetCurve(){
